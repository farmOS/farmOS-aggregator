...
instances. The application provides a GUI for managing farmOS instance credentials within the application
and a REST API for interacting with farmOS instsances.
...
### Manage farms
![Manage Farms](img/ui_manage_farms.png)
![UI Dashboard](img/ui_add_farm.png)
...
Thanks to FastAPI, the API endpoints are autogenerated with OpenAPI specs at [/docs](https://localhost/docs). Images of the OpenAPI documentation [here](#openapi-api-docs)
...
The root `farms` endpoint is used for managing farmOS. It supports HTTP `GET`, `POST`, `PUT` and `DELETE` requests.
To connect a farmOS instance the `farm_name`, `url`, `username` and `password` fields are required. farmOS passwords are never returned via the API, and are never visible within the GUI.

**NOTE:** *farmOS passwords are stored in plain text within the DB.* This is required to interact with farmOS instances. In the future, farmOS will use OAuth and allow administraters of farmOS instances to assign farmOS-aggregator a token for authenticating farmOS instances.
...
All of the endpoints used for interacting with farmOS instances are namespaced from the `/api/v1/farms` endpoint. All endpoints support HTTP `GET`, `POST`, `PUT` and `DELETE` requests, with the exception of `/farms/info` which only supports `GET` requests. By default, all endpoints will attempt to run queries on **all** farmOS instances saved in the aggregator. Passing a list of `farm_id` as a `farms` query parameters, with any request, will only perform actions from the listed farms. For example, a `GET` to `/api/v1/farms/info?farms=1&farms=5` will only return the farmOS instance info from farms of ID `1` and `5`. The `farm_id` of farmOS instances can be retrieved with a `GET` to `/api/v1/farms`.
...
![Farms APIs](img/farm_all_api.png)

All farmOS related APIs are listed under the `farm` tag prefix.

![Farm Create](img/farm_create.png)

The OpenAPI docs are interactive, allowing to perform requests from the "Try it out" button. This displays the fields required to perform requests at each endpoint.
