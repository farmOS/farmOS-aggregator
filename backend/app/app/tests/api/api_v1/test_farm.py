...
def test_create_delete_farm(
    client: TestClient, db: Session, farm_create_headers, farm_delete_headers
):
    ...
    r = client.post(
        f"{settings.API_V1_PREFIX}/farms/", headers=farm_create_headers, json=data,
    )
    ...
    assert (
        ...
        == created_farm["token"]["access_token"]
        == token["access_token"]
    )
    assert (
        farm.token.expires_in
        == created_farm["token"]["expires_in"]
        == token["expires_in"]
    )
    assert (
        farm.token.refresh_token
        == created_farm["token"]["refresh_token"]
        == token["refresh_token"]
    )
    assert (
        float(farm.token.expires_at)
        == created_farm["token"]["expires_at"]
        == token["expires_at"]
    )

    # Delete the farm
    r = client.delete(
        f"{settings.API_V1_PREFIX}/farms/{farm.id}", headers=farm_delete_headers,
    )
    assert 200 <= r.status_code < 300
...
def test_create_farm_update_token(
    ...
    assert (
        farm.token.access_token
        == updated_farm["token"]["access_token"]
        == token["access_token"]
    )
    assert (
        farm.token.expires_in
        == updated_farm["token"]["expires_in"]
        == token["expires_in"]
    )
    assert (
        farm.token.refresh_token
        == updated_farm["token"]["refresh_token"]
        == token["refresh_token"]
    )
    assert (
        float(farm.token.expires_at)
        == updated_farm["token"]["expires_at"]
        == token["expires_at"]
    )
...
def test_create_farm_delete_token(
    client: TestClient,
    db: Session,
    farm_create_headers,
    farm_update_headers,
...
def test_get_all_farms(client: TestClient, db: Session, test_farm, farm_read_headers):
    ...
    r = client.get(f"{settings.API_V1_PREFIX}/farms/", headers=farm_read_headers,)
    assert 200 <= r.status_code < 300
    response = r.json()
...
def test_get_farm_by_id(client: TestClient, db: Session, test_farm, farm_read_headers):
    farm_id = test_farm.id
    r = client.get(
        f"{settings.API_V1_PREFIX}/farms/{farm_id}", headers=farm_read_headers,
    )
    assert 200 <= r.status_code < 300
    response = r.json()
...
def test_farm_create_oauth_scope():
    ...
    app.dependency_overrides[utils.get_settings] = settings_closed_registration
    r = client.post(
        f"{settings.API_V1_PREFIX}/farms/", headers={"api-token": token.decode("utf-8")}
    )
    assert r.status_code == 401
